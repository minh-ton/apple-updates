...
module.exports = async function () {
    this.fetch_macos_xml = async function (url, dname) {
        let response = await axios.get(url).catch(function (error) {
            return log_error(error, "xml.js", `fetch_macos_pkg - ${dname}`, `fetching the catalog for new updates`);
        });

        if (!response) return log_error("No XML response availble.", "xml.js", `fetch_macos_pkg - ${dname}`, `fetching the catalog for new updates`);
        ...
        for (let product in catalog_content.Products) for (let package in catalog_content.Products[product].Packages) if (catalog_content.Products[product].Packages[package].URL.endsWith('InstallAssistant.pkg')) {
            ...
            let info = await axios.get(pkg_info).catch(function (error) {
                return log_error(error, "xml.js", `fetch_macos_pkg - ${dname}`, `getting InstallAssistant.pkg info from the distribution file`);
            });

            if (!info) return log_error("No product info available.", "xml.js", `fetch_macos_pkg - ${dname}`, `getting InstallAssistant.pkg info from the distribution file`);
