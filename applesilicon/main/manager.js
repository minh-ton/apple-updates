...
module.exports = function () {
    this.fetch_macos_updates = async function (assetaud, dname, beta) {
        ...
        if (!mac_update) return send_error(`No asset available.`, "manager.js", `fetch_macos_ota`, `update not available for ${assetaud}.`);
        ...
        const pkgurl = mac_update['mac_pkg'];
        const version = mac_update['mac_version'];
        const size = mac_update['mac_size'];
        const build = mac_update['mac_build'];
        const updateid = mac_update['mac_updateid'];
        const changelog = mac_update['mac_changelog'];
    ...
    this.get_macos_assets = async function (assetaud, beta) {
        let mac_update = await gdmf_macos(assetaud, beta);
        return mac_update;
    };

    this.get_other_assets = async function (assetaud, os_build, hwm, prodtype, prodversion, cname, dname, beta)  {
        let os_update = await gdmf_other(assetaud, os_build, hwm, prodtype, prodversion, cname, dname, beta);
        return os_update;
    };
